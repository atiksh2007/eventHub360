...
let ApprovalService = class ApprovalService {
    ...
    onModuleInit() {
        this.qtnService = this.moduleRef.get(quotation_service_1.QuotationService, { strict: false });
    }
    async getApprovalDetails(quoteNumber) {
        const data = this.approvalCache[quoteNumber];
        if (!data) {
            throw new common_1.NotFoundException(`Approval track for ${quoteNumber} not found.`);
        }
        return data;
    }
    initializeApprovalTrack(params) {
        ...
        if (params.activeStepName === 'Draft') {
            workflowSteps.push({ name: "Approved Awaiting", status: "completed", dateLabel: "Auto-Approved" });
        }
        else if (params.activeStepName === 'Manager Review') {
            workflowSteps.push({ name: "Manager Review", status: "current", dateLabel: "In Progress" }, { name: "Finance Review", status: "upcoming" }, { name: "Approved Awaiting", status: "upcoming" });
        }
        else {
            workflowSteps.push({ name: "Owner Review Exception", status: "current", dateLabel: "In Progress" }, { name: "Finance Review", status: "upcoming" }, { name: "Approved Awaiting", status: "upcoming" });
        }
        const newApproval = {
    ...
    cancelApprovalWorkflow(quoteNumber) {
        const data = this.approvalCache[quoteNumber];
        if (data) {
            const formattedTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    ...
    async processWorkflowAction(quoteNumber, action, feedback) {
        const data = this.approvalCache[quoteNumber];
        if (!data) {
            throw new common_1.NotFoundException(`Approval track for ${quoteNumber} not found.`);
        }
        ...
        if (!rawQuote) {
            throw new common_1.NotFoundException(`Linked Quotation workspace ${quoteNumber} not found.`);
        }
        const formattedTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
